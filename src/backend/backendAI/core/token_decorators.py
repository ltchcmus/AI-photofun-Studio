...
Usage:
    from core.token_decorators import require_tokens, track_processing_time
    
    # Check balance only (deduct after processing based on time)
    @track_processing_time(feature="image_generation", min_required_tokens=10)
    def generate_image_view(request):
        # Processing time is tracked, tokens deducted after completion
        return Response(...)
