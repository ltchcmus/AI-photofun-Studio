...
public class WebSocket {
    ...
    public WebSocket(SocketIOServer server, CommunicationService communicationService, Utils utils, IdentityClient identityClient) {
        this.server = server;
        ...
        this.server.addEventListener("sendMessage", SendMessageRequest.class, this::sendMessage);
        ...
        this.server.addEventListener("sendMessageToGroup", SendMessageGroupRequest.class, this::sendMessageToGroup);
    }

    private void onConnect(SocketIOClient client) {
        log.info("New client connected: " + client.getSessionId());
        sessionToUserId.put(String.valueOf(client.getSessionId()),client.getHandshakeData().getSingleUrlParam("userId"));
        userIdToClient.put(client.getHandshakeData().getSingleUrlParam("userId"),client);

        List<String> groupJoined = identityClient.getGroupJoinedInternal(client.getHandshakeData().getSingleUrlParam("userId")).getResult();
    ...
    private  void onDisconnect(SocketIOClient client) {
        ...
        List<String> groupJoined = identityClient.getGroupJoinedInternal(userId).getResult();
        for (String groupId : groupJoined) {
            client.leaveRoom(groupId);
        }
    }

    private void sendMessage(SocketIOClient client, SendMessageRequest data, AckRequest ackRequest) {
    ...
    private void join(SocketIOClient client, String groupId, AckRequest ackRequest) {
        client.joinRoom(groupId);
    }
    private void leave(SocketIOClient client, String groupId, AckRequest ackRequest) {
        client.leaveRoom(groupId);
    }

    private void sendMessageToGroup(SocketIOClient client, SendMessageGroupRequest data, AckRequest ackRequest) {
